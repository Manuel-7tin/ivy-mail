...
const uploadCsvPara=document.querySelector('#upload-csv-p')
...
const csvForm = document.querySelector('.submit-form')

// smsBody.style.display='none'
// bothBody.style.display='none'
...
// if(screenWidth>850){
//    alert('yam')
//    emailBody.classList.add('js-class-grid')
// }
})
        ...
        csvInputFile.onchange = function(){
           if(csvInputFile.files.length>0){
               for(let i=0; i < csvInputFile.files.length; i++){
                   //  alert(csvInputFile.files.item(i).name)
                   ...
                   single.previousElementSibling.innerHTML=`${toDisplay}...`
                       ...
                       single.previousElementSibling.innerHTML=`${arr.join().slice(0, 20)}...`
                   }
